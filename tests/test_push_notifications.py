...
@pytest.fixture
def backend(monkeypatch: pytest.MonkeyPatch) -> FakePushNotificationsBackend:
    fake = FakePushNotificationsBackend()
...
@pytest.mark.asyncio
async def test_get_push_state(config: BydConfig, backend: FakePushNotificationsBackend) -> None:
    async with BydClient(config) as client:
        state = await client.get_push_state(backend.vin)
        assert state.is_enabled is True
        assert state.push_switch == 1
        assert state.vin == backend.vin
    assert backend.calls.get("/app/push/getPushSwitchState", 0) == 1


@pytest.mark.asyncio
async def test_get_push_state_api_error(config: BydConfig, backend: FakePushNotificationsBackend) -> None:
    backend.get_error_code = "9999"
    async with BydClient(config) as client:
        with pytest.raises(BydApiError, match="getPushSwitchState"):
            await client.get_push_state(backend.vin)


@pytest.mark.asyncio
async def test_set_push_state_enable(config: BydConfig, backend: FakePushNotificationsBackend) -> None:
    async with BydClient(config) as client:
        result = await client.set_push_state(backend.vin, enable=True)
        assert result == {"result": "ok"}
    assert backend.calls.get("/app/push/setPushSwitchState", 0) == 1


@pytest.mark.asyncio
async def test_set_push_state_disable(config: BydConfig, backend: FakePushNotificationsBackend) -> None:
    async with BydClient(config) as client:
        result = await client.set_push_state(backend.vin, enable=False)
        assert result == {"result": "ok"}


@pytest.mark.asyncio
async def test_set_push_state_api_error(config: BydConfig, backend: FakePushNotificationsBackend) -> None:
    backend.set_error_code = "9999"
    async with BydClient(config) as client:
        with pytest.raises(BydApiError, match="setPushSwitchState"):
            await client.set_push_state(backend.vin, enable=True)
